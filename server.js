...
async function safeReadJSON(file, fallback){
  ...
  catch(e){ console.warn("safeReadJSON failed", e); return fallback; }
}
...
// ensure files exist
(async ()=>{
  try { await fs.access(TRADES_FILE); } catch(e){ console.warn("TRADES_FILE not found", e); await safeWriteJSON(TRADES_FILE, []); }
  try { await fs.access(ACCOUNT_FILE); } catch(e){ console.warn("ACCOUNT_FILE not found", e); await safeWriteJSON(ACCOUNT_FILE, { balance: 10000, equity: 10000, usedMargin: 0 }); }
})();
...
async function fetchPrices(){
  ...
  for(const s of syms){
    ...
    }catch(e){ console.warn("server.js: order processing error", e); }
  }
}
