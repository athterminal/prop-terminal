...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.formatCandleTime = function (tsSec, tf) {
    ...
    const hh = pad(d.getUTCHours());
    const mm = pad(d.getUTCMinutes());
    const dow = weekdays[d.getUTCDay() === 0 ? 6 : d.getUTCDay()-1];
    const base = `${dow} ${day} ${month} ’${year}`;
    if (["1M"].includes(tf)) {
      const first = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1));
      const fDow = weekdays[first.getUTCDay() === 0 ? 6 : first.getUTCDay()-1];
      return `${fDow} 01 ${months[first.getUTCMonth()]} ’${String(first.getUTCFullYear()).slice(2)}`;
    }
    if (["1w"].includes(tf)) {
      const dd = new Date(d);
      const dowNum = dd.getUTCDay() === 0 ? 6 : dd.getUTCDay()-1;
      dd.setUTCDate(dd.getUTCDate() - dowNum);
      const wDow = weekdays[dd.getUTCDay() === 0 ? 6 : dd.getUTCDay()-1];
      const wDay = pad(dd.getUTCDate());
      const wMonth = months[dd.getUTCMonth()];
      const wYear = String(dd.getUTCFullYear()).slice(2);
      return `${wDow} ${wDay} ${wMonth} ’${wYear}`;
    }
    if (["1d"].includes(tf)) return base;
    if (["4h","1h","15m","5m","1m"].includes(tf)) return `${base} ${hh}:${mm}`;
    return base;
  };
  ...
  if (window.chart) {
    chart.applyOptions({ localization: { timeFormatter: (t)=>window.formatCandleTime(t, window.currentInterval||"1h") } });
  }
});
